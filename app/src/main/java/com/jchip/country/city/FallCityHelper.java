...
public abstract class FallCityHelper {
    ...
    init(countryCode: String) {
        if(!countryCode.isEmpty && self.info.isEmpty) {
            if let path = Bundle.main.path(forResource: "city_\(countryCode.lowercased())", ofType: "csv") {
                do {
                    //let data = try String(contentsOfFile: path, encoding: .windowsCP1252)
                    ...
                    for line in lines {
                        if(!line.isEmpty) {
                            ...
                            for item in items {
                                cityInfo.append(item.replacingOccurrences(of: "\"", with: ""))
                            }
                            self.info.append(cityInfo)
                        }
                    }
    ...
    public static boolean isSortable(int sortIndex, boolean isPortrait) {
        if (isPortrait) {
            if (sortIndex == FallCityHelper.SORT_OFFICIAL || sortIndex == FallCityHelper.SORT_ALPHA_3 || sortIndex == FallCityHelper.SORT_NUMERIC || sortIndex == FallCityHelper.SORT_SYMBOL || sortIndex == FallCityHelper.SORT_POPULATION) {
                return false;
            }
        } else {
            if (sortIndex == FallCityHelper.SORT_COUNTRY) {
                return false;
            }
        }
        return true;
                ...
                for (String item : info.keySet()) {
                    if (!searchInfo.contains(item)) {
                        ...
                        if (searchIndex == COUNTRY || searchIndex == OFFICIAL || searchIndex == CAPITAL) {
                            String prefix = (searchIndex == FallCountryViewHelper.CAPITAL ? "capital" : (searchIndex == FallCountryViewHelper.OFFICIAL ? "official" : "short"));
        ...
        public int compare(String key1, String key2) {
            if (index == COUNTRY || index == OFFICIAL || index == CAPITAL) {
                if (context != null && context.getResources() != null) {
                    String item1 = (index == FallCountryViewHelper.CAPITAL ? "capital_" : (index == FallCountryViewHelper.OFFICIAL ? "official_" : "short_")) + key1.toLowerCase();
                    String item2 = (index == FallCountryViewHelper.CAPITAL ? "capital_" : (index == FallCountryViewHelper.OFFICIAL ? "official_" : "short_")) + key2.toLowerCase();
                ...
                int value1 = FallUtility.parseInteger(info.get(key1)[index], 0);
                int value2 = FallUtility.parseInteger(info.get(key2)[index], 0);
